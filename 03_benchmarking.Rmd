...
>sp|P0DKJ0|P160B_ARATH	Peptide encoded by miPEP160b OS=Arabidopsis thaliana OX=3702 GN=miPEP160b PE=4 SV=1
MFSPQ in ampin33.fasta excluded

```{r}
ampgram_homo_filepaths <- c(list.files("data/prediction_results/ampgram/homo", pattern="*.rds",full.names = T))

gimme <- function(filepath) {
  
  ampgram_homo_list <- readRDS(filepath)

  amgram_probandnamelist <- lapply(ampgram_homo1, '[[', 3)

  bind_rows(amgram_probandnamelist, .id = "ID")
}

tst <- map_df(ampgram_homo_filepaths, gimme)
