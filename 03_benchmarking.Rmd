...
```{r, echo = FALSE}
knitr::kable(proteome_metrics[,c(3,6, 4:5, 7:11)], digits = 2)
```
...
ggplot(proteome_rocs) + 
  ...
  theme(legend.position = "bottom",
        strip.text = element_text(face = "italic"),
        strip.background = element_blank()) +
  scale_colour_manual(breaks = c("ampir_v1.1_precursor_imbal", "ampir_v1.1_mature","ampir_v1.1prec_model_nobench",
                                 "AMPscanner_v2", "amPEP", "AmpGram", "amPEPpy", "AMPlify"),
                     labels = model_labels,
                     values = model_col_values) +
   guides(colour = guide_legend(nrow = 1))
