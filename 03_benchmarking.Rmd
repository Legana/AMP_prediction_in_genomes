...
reference_proteomes <- c("data/proteomes/uniprot-proteome UP000005640.tab","data/proteomes/uniprot-proteome UP000006548.tab") %>% 
  map_dfr( ~ read_tsv(.x) %>% mutate(Label = case_when(str_detect(`Keyword ID`, "KW-0929") ~ "Pos", TRUE ~ "Neg"))) %>% 
...
if ( file.exists("data/prediction_results/ampir/ampirv1.1_refproteomes_predictions.rds")){
  ...
  ampir_models <- list("ampir_v1.1_precursor_imbal"=ampirv1.1_precursor_imbal,
                       "ampir_v1.1_precursor_bal"=ampirv1.1_precursor_bal,
                       "ampir_v1.1_mature"=ampirv1.1_mature,
                       "ampir_v1.1prec_model_nobench"=ampir_v1.1prec_model_nobench)
  ...
  run_ampir_models_for_proteome <- function(organism_name){
    names(ampir_models) %>% map_dfr( ~ reference_proteomes %>% 
                                          ...
                                          select(`Entry name`, Sequence) %>% 
                                          as.data.frame() %>% 
                                          predict_amps(n_cores = 3, model = ampir_models[[.x]]) %>% 
                                          add_column(Model = .x)
                                          )
  }
  
  # These are quite time consuming (~10 minutes on a Mac Mini)
...
Something here about the test data. Was it identical to the data mentioned in Table 1.1? When was it run? Was the webserver used to make predictions?
...
ampscan_genome_bench <- list.files("data/prediction_results/ampscanner_v2", pattern="[ha][ro][am].*.csv",full.names = T) %>% 
  map_dfr( ~ read_csv(.x) %>% separate(SeqID,into = c("database","Entry","Entry name"),sep = "\\|") ) %>% 
  left_join(reference_proteomes,by="Entry name") %>% 
...
ampscan_pub_bench <- list.files("data/prediction_results/ampscanner_v2", pattern="amps_.*.csv",full.names = T) %>% 
  map_dfr( ~ read_csv(.x) %>% add_column(Label = ifelse(grepl("nonamps",.x),"Neg","Pos"))) %>% 
...
ampep_pub_bench <- read_csv("data/prediction_results/ampep/xiao_independent_ampep.txt") %>% 
...
amPEPpy was run according to the example provided on the [amPEPpy GitHub repository](https://github.com/tlawrence3/amPEPpy) in a Conda environment (commands shown below). 
...
ampeppy_pub_bench <-  read_tsv("data/prediction_results/amPEPpy/xiao_independent_ampeppy_results.tsv") %>% 
...
amplify_genome_bench <- c("data/prediction_results/AMPlify1.0.0/AMPlify_Athaliana.txt","data/prediction_results/AMPlify1.0.0/AMPlify_Hsapiens.txt") %>% 
  map_dfr( ~ read_delim(.x,delim = "\t", col_names = c("Entry name", "prob_AMP")) %>% 
...
```{r}
ampir_genome_roc <- do.call(rbind,lapply(c("ampir_v1.1_precursor_imbal","ampir_v1.1_precursor_bal","ampir_v1.1_mature", "ampir_v1.1prec_model_nobench"),function(meth){
...
rocplot <- ggplot(proteome_rocs) + 
  ...
  theme(legend.position = "bottom",
        ...
        strip.text = element_text(face = "italic",
                                  size = 10)) +
 scale_colour_manual(breaks = c("ampir_v1.1_precursor_imbal","ampir_v1.1_precursor_bal","ampir_v1.1_mature", "ampir_v1.1prec_model_nobench",
                                "AMPscanner_v2", "amPEP", "AmpGram", "amPEPpy", "AMPlify"),
                     labels = c("ampir_prec_imbal\nH .92 A .99","ampir_prec_bal","ampir_mature","ampir_prec_nb",
                                "AMPscanner\nH .79 A .92", "amPEP\nH .43 A .16", "AmpGram\nH .81 A .86", "amPEPpy\nH .49 A .24",
                                "AMPlify\n H .67 A .62"),
                     values = c("blueviolet", "goldenrod2", "darkslategray","black", "cyan", "green", "darkorange3", "grey50", "deeppink")) +
...
ampir_metrics <- do.call(rbind, lapply(c("ampir_v1.1_precursor_imbal","ampir_v1.1_precursor_bal","ampir_v1.1_mature", "ampir_v1.1prec_model_nobench"),function(meth) {
...
ggplot(proteome_metrics_long, aes(x = Organism, y = value)) +
  ...
  scale_fill_manual(breaks = c("ampir_v1.1_precursor_imbal", "ampir_v1.1_precursor_bal", "ampir_v1.1_mature","ampir_v1.1prec_model_nobench",
                               "AMPscanner_v2", "amPEP", "AmpGram", "amPEPpy", "AMPlify"),
                     labels = c("ampir_prec_imbal", "ampir_prec_bal", "ampir_mat","ampir_pred_nb","AMPscanner", "amPEP", "AmpGram", "amPEPpy", "AMPlify"),
                     values = c("blueviolet", "goldenrod2", "darkslategray","black", "cyan", "green", "darkorange3", "grey50", "deeppink")) +
