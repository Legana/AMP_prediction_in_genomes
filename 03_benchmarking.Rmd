...
ampgram_arab_preds_clean <- ampgram_arab_preds %>%
...
# how to add rows for missing predictions  ? eg arab missing 3 predictions, add NA value? or just leave it and hope its ok? check other prediction outcomes if its compatible
```
...
ampgram_homo_filepaths <- list.files(c("data/prediction_results/ampgram/homo", "data/prediction_results/ampgram/homo/leftovers"), pattern="*.rds",full.names = T)

ampgram_homo_preds <- map_df(ampgram_homo_filepaths, gimme_ampgram_predictions)
```
