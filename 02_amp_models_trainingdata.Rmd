...
**Figure 2.1:** The sequence length of AMP and non-AMP sequences in the training and testing set of various AMP prediction models. The training set for AmpGram and the testing sets for amPEP and amPEPpy are blank as these were not specified.

```{r, eval=FALSE, echo=FALSE}
ggsave("figures/model_lengths.png", width = 14, height = 13 ,units = "cm")
```
...
```{r, eval=FALSE, echo=FALSE}
ggsave("figures/model_lengths_combined.png", width = 12, height = 10 ,units = "cm")
```
...
ggplot(pca_proteomes_w_annotations, aes(x = PC1)) +
   stat_density(aes(colour = class), geom = "line", position = "identity") +
...
**Figure 2.4:** PCA of features of all AMP and non-AMP sequences (between 10-3000 amino acids long) in the proteomes *Homo sapiens* and *Arabidopsis thaliana* 


```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pca_proteome_feats.png", width = 13, height = 9 ,units = "cm")
```
...
ggplot(filter(pca_proteomes_w_annotations, Status == "reviewed"), aes(x = PC1)) +
   stat_density(aes(colour = class), geom = "line", position = "identity") +
   facet_wrap(~factor(Organism, levels = c("Homo sapiens (Human)", "Arabidopsis thaliana (Mouse-ear cress)"), labels = c("H. sapiens", "A. thaliana")),       scales = "free_y") +
   labs(x = pca_prot_percentages[1], y = "Density", colour = "") +
   theme_classic() +
...
```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pca_proteome_feats_reviewed.png", width = 13, height = 9 ,units = "cm")
```
