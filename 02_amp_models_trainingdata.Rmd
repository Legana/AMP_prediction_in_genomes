...
# Perform feature calculation with ampir if needed. Otherwise read cached values.

if ( file.exists("cache/all_predictor_data_feats.rds")){
  all_predictor_data_feats <- readRDS("cache/all_predictor_data_feats.rds")
} else {
  all_predictor_data_feats <-  calculate_features(all_predictor_data,min_len = 10) %>% 
    cbind(all_predictor_data %>% select(-seq_name)) %>% 
    select(seq_name:Xc2.lambda.19,class,name=predictor,length,dataset)
  
  saveRDS(all_predictor_data_feats, "cache/all_predictor_data_feats.rds")
}

if ( file.exists("cache/reference_proteome_feats.rds")){
  reference_proteomes_feats <- readRDS("cache/reference_proteome_feats.rds")
} else {
  reference_proteomes_feats <- reference_proteomes %>%
    select(`Entry name`, Sequence) %>%
    as.data.frame() %>% 
    calculate_features() %>% 
    cbind(reference_proteomes) %>%
    ...
    select(seq_name:Xc2.lambda.19,class,name,length=Length,dataset)
  
  saveRDS(reference_proteomes_feats, "cache/reference_proteome_feats.rds")
}

ampep_test <- all_predictor_data_feats %>% 
  filter(name=="iAMP-2L") %>% 
  filter(dataset=="Test") %>% 
  mutate(name="AmPEP")

ampeppy_test <- all_predictor_data_feats %>% 
  filter(name=="AmPEPpy") %>% 
...
name_to_factor <- function(data){
  data %>% 
  mutate(name = factor(name, levels = c("iAMP-2L", "AMP Scanner v2", "AmPEP", "AmPEPpy", "deep_AmPEP", "AMPlify", "AmpGram", "ampir_precursor", "ampir_mature", "Homo sapiens", "Arabidopsis thaliana")))
}

predictor_and_proteome_feats <- rbind(all_predictor_data_feats, reference_proteomes_feats) %>% name_to_factor

predictor_and_proteome_feats_tetr <- rbind(all_predictor_data_feats, ampep_test,ampeppy_test, reference_proteomes_feats) %>% name_to_factor
```
...
model_seqlength_tr <- 
  predictor_and_proteome_feats_tetr %>% 
...
**Figure Supp_2.1:** Comparison of sequence length distributions for positive (AMP; purple) and negative (non-AMP; green) fractions in test and training data eight AMP predictors.  
...
model_seqlength <- predictor_and_proteome_feats %>% 
  ggplot() +
  geom_histogram(aes(fill=class,x=length),position="dodge",binwidth=5) +
  facet_wrap(~name, ncol=1,scales = "free_y") +
  labs(x = "Sequence length", y = "Number of sequences", fill = "") +
...
**Figure 2.2:** Comparison of sequence length distributions for positive (AMP; purple) and negative (non-AMP; green) fractions in merged (test and training) data for eight AMP predictors, and for the proteomes of *A. thaliana* and *H. sapiens*.
...
set_data <- amp_data_wide %>% 
  ...
  group_by(group) %>% 
  summarise(seq_aa=first(seq_aa),signalp=sum(signalp)>0,across(ampep:iamp2l,~ sum(.x)>0)) %>% 
  select(-group) %>% 
...
preds <- c("ampep","ampeppy","ampirmature","ampirprecursor","amplify","ampscan2","deepampep","iamp2l")
```
...
**Figure 2.4:** Sequence composition of 831 AMP sequences with length > 50 (likely precursors) in Uniprot showing the relative locations of Signal Peptide, Mature AMP and C-terminal sequences. 
