...
ggsave("figures/model_lengths_combined.png", width = 16, height = 14, units = "cm", dpi = 500)
```

## PCA and t-SNE of AMP and non-AMP features
...
```{r, echo = FALSE}
tsne_features <-readRDS("cache/tsne_AMPmodelsfeatures.rds")
```
...
```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pca_model_feats_tt.png", width = 18, height = 16 ,units = "cm")
```
...
```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pca_model_feats.png", width = 18, height = 17 ,units = "cm")
```
...
*some issues here. in order to use tSNE, you have to use "unique()" which removes stuff. in this case it removed quite a bit, including all the non-AMPs for deep_ampep...*
```{r}
...
**Test Figure:** tSNE of features of AMP and non-AMP sequences used in various AMP prediction models. 
...
**Test Figure:** tSNE of features of AMP and non-AMP sequences used in ampir.



## PCA and tSNE on reference proteomes
...
```{r, echo =FALSE}
tsne_proteomes <- readRDS("cache/tsne_proteomes.rds")
```
...
tsne_proteomes_amps <- filter(tsne_proteomes_annotated, class == "AMP")
tsne_proteomes_nonamps <- filter(tsne_proteomes_annotated, class == "non-AMP")

tsne_proteomes_plot <- ggplot(tsne_proteomes_annotated) +
         geom_point(data = tsne_proteomes_nonamps, aes(x = tSNE_1, y = tSNE_2, colour = class, shape = class)) +
         geom_point(data = tsne_proteomes_amps, aes(x = tSNE_1, y = tSNE_2, colour = class, shape = class)) +
         facet_wrap(~factor(Organism, levels = c("Homo sapiens (Human)", "Arabidopsis thaliana (Mouse-ear cress)"), labels = c("H. sapiens", "A. thaliana")),
         scales = "free_y") +
...
(pca_proteomes_plot | pca1_proteomes_plot) / tsne_proteomes_plot + plot_annotation(tag_levels = 'A')

```

**Figure 2.4:** PCA and tSNE of features of all AMP and non-AMP sequences (between 10-3000 amino acids long) in the proteomes *Homo sapiens* and *Arabidopsis thaliana* 

```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pcatsne_proteomes.png", width = 17, height = 12 ,units = "cm")
```
...
tsne_proteomes_plot_rev <- ggplot(tsne_proteomes_annotated) +
         geom_point(data = filter(tsne_proteomes_nonamps, Status == "reviewed") , aes(x = tSNE_1, y = tSNE_2, colour = class, shape = class)) +
         geom_point(data = filter(tsne_proteomes_amps, Status == "reviewed"), aes(x = tSNE_1, y = tSNE_2, colour = class, shape = class)) +
         facet_wrap(~factor(Organism, levels = c("Homo sapiens (Human)", "Arabidopsis thaliana (Mouse-ear cress)"), labels = c("H. sapiens", "A. thaliana")),
         scales = "free_y") +
         labs(x = "tSNE 1", y = "tSNE 2", colour = "", shape = "") +
         theme_classic() +
...
(pca_proteomes_plot_rev | pca1_proteomes_plot_rev) / tsne_proteomes_plot_rev + plot_annotation(tag_levels = 'A')
```

**Figure 2.5:** PCA of features of **reviewed** AMP and non-AMP sequences (between 10-3000 amino acids long) in the proteomes *Homo sapiens* and *Arabidopsis thaliana* 

```{r, eval=FALSE, echo=FALSE}
ggsave("figures/pcatsne_proteomes_reviewed.png", width = 17, height = 12 ,units = "cm")
```
