...
ggplot(pca_values, aes(x = PC1, y = PC2)) +
   ...
   theme(legend.position = "bottom",
        ...
        panel.grid.minor = element_blank(),
        panel.grid.major = element_blank()) +
        scale_colour_manual(labels = c("AMP", "non-AMP"),
                            values = c("darkblue", "brown4")) +
...
```{r, echo=FALSE, eval=FALSE}
saveRDS(pca_proteomes_x, "cache/pca_proteomes_values.rds")
```

```{r, echo = FALSE}
pca_proteomes_x <- readRDS("cache/pca_proteomes_values.rds")
```
...
ggplot(pca_proteomes_w_annotations, aes(x = PC1, y = PC2)) +
   geom_point(aes(colour = class, shape = class)) +
   facet_grid(~Organism, scales = "free_y") +
   labs(x = pca_prot_percentages[1], y = pca_prot_percentages[2], colour = "") +
   theme(legend.position = "bottom",
        #strip.text.y.right = element_text(angle = 0, hjust = 0),
        strip.text.y.right = element_blank(),
        strip.background = element_blank(),
