...
``` r
metazoa_featAMPs_subset <- metazoa_feat %>% filter(Label == "Pos") %>% slice_sample(n = 618)
metazoa_featnonAMPs_subset <- metazoa_feat %>% filter(Label == "Neg") %>% slice_sample(n = 6180)
metazoa_feat_subset <- rbind(metazoa_featAMPs_subset, metazoa_featnonAMPs_subset)

trainIndex2 <-createDataPartition(y = metazoa_feat_subset$Label, p=.832, list = FALSE)
features_metazoaTrain2 <- metazoa_feat_subset[trainIndex2,]
features_metazoaTest2 <- metazoa_feat_subset[-trainIndex2,]
```
...
|    0.950 |       0.990 |  0.539 |     0.846 | 0.658 | 0.652 | 0.889 | 0.739 | Metazoa2 | Metazoa2 |
|    0.901 |       0.985 |  0.107 |     0.423 | 0.171 | 0.175 | 0.746 | 0.263 | Metazoa2 | Plants1  |
...
```
    ...
    ## D = 3.1934, df = 1281.6, p-value = 0.00144
...
```
    ...
    ## D = 5.5707, df = 2037.3, p-value = 2.873e-08
    ## alternative hypothesis: true difference in AUC is not equal to 0
    ## sample estimates:
    ## AUC of roc1 AUC of roc2 
    ##   0.8887424   0.7462008
...
Metazoa\_svm1 on metazoa1 data vs. Metazoa\_svm2 on metazoa2 data \|
0.00144 \|  
Metazoa\_svm1 on metazoa1 data vs. Metazoa\_svm2 on plant data \| &lt;
2.2 x 10<sup>-16</sup> \|  
Metazoa\_svm2 on metatazoa2 data vs. Metazoa\_svm2 on plant data \| 2.9
x 10<sup>-08</sup> \|
...
![](05_taxonomic_bias_effect_files/figure-gfm/unnamed-chunk-71-1.png)<!-- -->
